...
- $MLP_x$ 为共享节点投影网络；
- $\gamma_{\tau(v)},\beta_{\tau(v)}$ 为类型文本动态生成的调制参数。
...
| 类型调制参数 $\gamma_{\tau},\beta_{\tau}$ | 是 | 非独立参数 | 由节点类型文本动态生成 |
| 共享消息矩阵 $W_0$ | 否 | 是 | 提供通用图传播能力 |
| 关系参数生成器 $MLP_{\text{rel}}$ | 否 | 是 | 学习如何由关系文本生成调制 |
| 关系调制参数 $\gamma_r,\beta_r$ | 是 | 非独立参数 | 针对每种关系在 forward 中生成 |
